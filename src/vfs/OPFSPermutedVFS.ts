...
interface Transaction {
  txId: number;
  pages?: Map<number, { offset: number; digest: Uint32Array }>;
  fileSize?: number;
  oldestTxId?: number;
  reclaimable?: number[];
}
...
async function getPathComponents(path: string, create: boolean): Promise<[FileSystemDirectoryHandle, string]> {
...
class File {
  ...
  broadcastReceived: (Transaction | AccessRequest)[];

  mapPageToOffset: Map<number, number>;
  mapTxToPending: Map<number, Transaction>;
  freeOffsets: Set<number>;

  lockState: number;
  locks: { read?: () => void; write?: () => void; reserved?: () => void; hint?: () => void };

  abortController: AbortController;

  txActive: Transaction | null;
  txRealFileSize: number;
  txIsOverwrite: boolean;
  txWriteHint: boolean;
  ...
  static async create(pathname: string, flags: number): Promise<File> {
...
export class OPFSPermutedVFS extends FacadeVFS {
  ...
  static async create(name: string, module: any): Promise<OPFSPermutedVFS> {
  ...
  async jOpen(zName: string | null, fileId: number, flags: number, pOutFlags: DataView): Promise<number> {
    ...
    try {
      ...
      if (flags & VFS.SQLITE_OPEN_MAIN_DB) {
        ...
        try {
          const transactions: Transaction[] = await idbX(tx.objectStore('pending').getAll());
  ...
  async jLock(fileId: number, lockType: number): Promise<number> {
    ...
    switch (lockType) {
      ...
      case VFS.SQLITE_LOCK_RESERVED:
        ...
        const entries: Transaction[] = await idbX(tx.objectStore('pending').getAll(range));
  ...
  #lock(file: File, name: 'read' | 'write' | 'reserved' | 'hint', options: LockOptions = {}): Promise<boolean> {
  ...
  #processBroadcasts(file: File): void {
    ...
    for (const message of file.broadcastReceived) {
      if (Object.hasOwn(message, 'txId')) {
        const messageTx = message as Transaction;
  ...
  #acceptTx(file: File, message: Transaction): void {
  ...
  async #prepareOverwrite(file: File): Promise<void> {
    file.locks.read?.();
...
function checksum(data: ArrayBufferView): Uint32Array {
